...
public class BlogApiController {
    ...
    public ResponseEntity<Article> addArticle(@RequestBody @Validated AddArticleRequest request){
        Article savedArticle = blogService.save(request,getCurrentUsername());
    ...
    public ResponseEntity<Void> deleteArticle(@PathVariable(value="id") long id){
        blogService.delete(id,getCurrentUsername());
    ...
    public ResponseEntity<Article> updateArticle(@PathVariable(value="id") long id, @RequestBody UpdateArticleRequest request){
        Article updatedArticle = blogService.update(id, request,getCurrentUsername());
        return  ResponseEntity.ok().body(updatedArticle);
    }
    @PostMapping("/api/comments")
    public ResponseEntity<AddCommentResponse> addComment(@RequestBody AddCommentRequest request){
        Comment savedComment = blogService.addComment(request, getCurrentUsername());
